...
const PriceCalculator = ({
  initialMaterial = "Acrylic",
  initialSize = "",
  productHasLight = false,
  onPriceChange,
  externalDimensions = null,
}) => {
  ...
  const [height, setHeight] = useState(
    externalDimensions?.height || defaults.h,
  );
  const [width, setWidth] = useState(externalDimensions?.width || defaults.w);
  ...
  // Sync with external dimensions from editor
  useEffect(() => {
  ...
  const calculatePrice = useCallback(() => {
    ...
    const isAcrylicOrWood =
      mat.includes("acrylic") || mat.includes("wood") || mat.includes("wooden");
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mt-4">
      ...
      {/* Size inputs - hidden when dimensions come from editor */}
      {!externalDimensions && (
